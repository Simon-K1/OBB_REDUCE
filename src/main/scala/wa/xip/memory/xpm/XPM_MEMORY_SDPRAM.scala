...
class xpm_memory_sdpram(
    ...
    addGeneric("ADDR_WIDTH_A", ADDR_WIDTH_A)            //泛型参数
...
class sdpram(
    ...
    /**
     * xpm_memory_sdpram : Xilinx XPM (Xilinx Parameterized Macro：Xilinx公司提供的一种IP核库，用于 FPGA 设计。这个库包含了一系列参数化的宏，用于实现各种不同功能的IP核。)中的一个宏，
     */

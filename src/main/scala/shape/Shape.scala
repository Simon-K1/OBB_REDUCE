...
//    println(mulDataCount1.io.P.getWidth)
    ...
    val channelTimes0 = (instructionReg(Instruction.CHANNEL_IN) >> log2Up(shapeConfig.COMPUTE_CHANNEL_NUM)).asUInt.resize(instructionReg(Instruction.CHANNEL_IN).getWidth)
    val channelTimes1 = (instructionReg(Instruction.CHANNEL_IN1) >> log2Up(shapeConfig.COMPUTE_CHANNEL_NUM)).asUInt.resize(instructionReg(Instruction.CHANNEL_IN).getWidth)
    switch(shapeState.io.state) {
        is(State.CONCAT) {
            ...
            channelOutTimes :=  RegNext(channelTimes0 + channelTimes1)
        }
    ...
    val s = Vec(Stream(UInt(io.sData(1).payload.getWidth bits)), 2)
    s.foreach(a => a.ready := False)
    Vec(concat.dataPort.sData, maxPooling.io.sData, upSampling.io.sData) <> WaStreamDemux(Seq(State.CONCAT, State.MAX_POOLING, State.UP_SAMPLING), io.sData(0), shapeState.io.state)
    Vec(concat.sData1, s(0), s(1)) <> WaStreamDemux(Seq(State.CONCAT, State.MAX_POOLING, State.UP_SAMPLING), io.sData(1), shapeState.io.state)

    fifo.io.push <> WaStreamMux(Seq(State.CONCAT, State.MAX_POOLING, State.UP_SAMPLING), shapeState.io.state, Seq(concat.dataPort.mData.s2mPipe().m2sPipe(), maxPooling.io.mData, upSampling.io.mData.s2mPipe().m2sPipe()))
    //    switch(shapeState.io.state) {
    //        is(State.CONCAT) {
